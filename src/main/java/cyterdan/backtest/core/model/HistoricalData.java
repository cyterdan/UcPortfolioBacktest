...
/**
 *
 * Encapsulate historical funds data
 * 
 * @author cytermann
 */
public final class HistoricalData {
    ...
    /**
     * euro funds are ~fixed return and so their historical data can be
     * extrapolated when missing
     */
     ...
     * @param data
     */
    public void merge(HistoricalData data) {
        for (Entry<String, DailySerie> entry : data.series()) {
            this.history.put(entry.getKey(), entry.getValue());
        }
    }

    /**
     * fake "cash" fund with 1% yearly return
     */
    ...
    /**
     * remove all funds that do not have daily variations
     */
